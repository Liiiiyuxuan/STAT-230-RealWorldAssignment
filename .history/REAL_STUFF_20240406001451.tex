...
\begin{table*}[ht]
    ...
    Strait Flush 
    \begin{tikzpicture}
        \foreach \rank/\suit/\x in {A/\(\heartsuit\)/0, K/\(\heartsuit\)/0.8, Q/\(\heartsuit\)/1.6, J/\(\heartsuit\)/1.5, 10/\(\heartsuit\)/2} {
        % Draw the card
        \draw (\x,0) rectangle ++(0.3,0.6);
        % Add the rank and suit
        \node at (\x+0.2,0.3) {\rank\suit};
        }
    \end{tikzpicture} & cell 2 \\
    Quad & cell 2 \\
    Full House & cell 2 \\
    Flush & cell 2 \\
    Straight & cell 2 \\ 
