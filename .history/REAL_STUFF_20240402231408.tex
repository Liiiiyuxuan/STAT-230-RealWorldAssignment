...
Out of the $\binom{52}{2}$ possible starting hands, there are $13 \times 
13$ distinct types of starting hands since there are 13 ranks, there are 
13 distinct pocket pairs (e.g. AA), 78 ($\binom{13}{2}$) distinct suited 
2-card combinations (e.g. AKs) and 78 ($\binom{13}{2}$) distinct unsuited 
2-card combinations (e.g. AKo). It is intended to calculate the pre-flop 
(before any community cards are dealt) win rate for each distinct 2-card 
combination. For each distinct combination, there are 
\[
    \left[ \; \prod_{i = 1}^{n - 1} \binom{52 - 2i}{2} \; \right \times 𝐶5 52−2𝑛
\] 
possible
