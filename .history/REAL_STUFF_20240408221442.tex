...
of calling, and use this to determine whether a call is profitable or not. By using our out odds as a rough substitute for our equity, or the chance that we win the pot, we can use the formula for expected value: $$E[X] = P(\text{winning}) \cdot \text{gain} - P(\text{losing}) \cdot \text{loss}$$
